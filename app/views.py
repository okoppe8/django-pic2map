...
from .GPS import get_lat_lon, get_exif_data
...
class ItemCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        imagefile = Image.open(item.image.path)
        exif_data = get_exif_data(imagefile)
        get_lat = get_lat_lon(exif_data)

        if get_lat[0]:
            item.geo_location = ','.join(map(str, get_lat))
        else:
            item.geo_location = ''

        if exif_data['DateTimeOriginal']:
            item.shooting_time = timezone.make_aware(
                datetime.strptime(exif_data['DateTimeOriginal'], '%Y:%m:%d %H:%M:%S'))
